...
public class C03_Locators {
    public static void main(String[] args) throws InterruptedException {
        ...
        List<WebElement> listlinkSayisi =driver.findElements(By.tagName("a"));
        System.out.println("Websayfasındaki link sayısı : "+ ((List<?>) listlinkSayisi).size());

      //Linklerin yazilarini yazdiralim
        //elimizdeki listedeki tum webelementlerin yazilarini for loop veya for each loop ile yazdirabiliriz
        //tum elementler yazdirilacaksa bu durumda for each loop daha kullanisli olacaktir
        for (WebElement each:listlinkSayisi
             ) {
